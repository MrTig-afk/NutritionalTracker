...
import {
  ...
  Trash2, BarChart3, CalendarDays, ChevronDown, ChevronUp, Save
} from "lucide-react";
...
// =============================================================================
// kJ → kcal CONVERSION
// Detects if a calorie value is likely in kJ (>600 is a strong signal for kJ)
// and converts accordingly. Applied at parse time before display.
// =============================================================================
function normalizeCalories(value) {
  const num = parseNumeric(value);
  if (num === null) return value;
  // If the raw string contains "kj" or "kilojoule" (case-insensitive), convert
...
function normalizeNutritionData(data) {
  ...
  if (result.calories !== undefined) {
    result.calories = normalizeCalories(result.calories);
  }
  return result;
}
...
// =============================================================================
// Resolve the best available nutrition data for logging
// Falls back from per_100g to per_serving intelligently
// =============================================================================
function resolveNutrition(nutrition) {
  if (!nutrition) return {};
  // If nutrition already is a flat serving object (from folder items saved pre-nested)
  if (!nutrition.per_serving && !nutrition.per_100g) return nutrition;
  // Prefer per_serving, fall back to per_100g
  if (nutrition.per_serving && Object.keys(nutrition.per_serving).length > 0) {
    return nutrition.per_serving;
  }
  if (nutrition.per_100g && Object.keys(nutrition.per_100g).length > 0) {
    return nutrition.per_100g;
  }
  return nutrition;
}

// =============================================================================
// IMAGE PIPELINE: crop → resize → grayscale → compress
// =============================================================================
...
function ImageCropper({ file, onConfirm, onCancel }) {
  ...
  const onDown = useCallback((e) => {
    ...
    const zone = getZone(pos, cropRect);
    if (!zone) return;
    isDragging.current = true; dragMode.current = zone;
    dragStart.current = pos; cropAtStart.current = { ...cropRect };
  }, [cropRect]);

  const onMove = useCallback((e) => {
    if (!isDragging.current || !cropAtStart.current) return;
    e.preventDefault();
    const pos = getPos(e, canvasRef.current);
...
function SaveToFolderModal({ result, imageId, onClose }) {
  ...
  useEffect(() => {
    apiFetch("/folders").then(setFolders).catch(() => {});
  }, []);
  ...
  // FIX: Ctrl+Backspace / Ctrl+Delete word-level editing in folder name input
  const handleFolderKeyDown = (e) => {
    if (e.key === "Enter") { e.preventDefault(); createFolder(); return; }
    if (e.ctrlKey && e.key === "Backspace") {
      e.preventDefault();
      setNewFolder(prev => prev.replace(/\s*\S+\s*$/, ""));
      return;
    }
    if (e.ctrlKey && e.key === "Delete") {
      ...
      const after = newFolder.slice(pos);
      const trimmed = after.replace(/^\s*\S+/, "");
      setNewFolder(newFolder.slice(0, pos) + trimmed);
    }
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const nutrition = result.per_serving
        ? { per_serving: result.per_serving, per_100g: result.per_100g }
        : result;
      await apiFetch(`/folders/${selectedFolder}/items`, {
        method: "POST",
        body: JSON.stringify({ name: itemName.trim(), image_id: imageId || "", nutrition }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
        ...
        {/* FIX: Enter creates folder, Ctrl+Backspace/Delete for word editing */}
        ...
        {status && (
          <p className={`text-xs font-mono ${status.type === "ok" ? "text-emerald-500" : "text-rose-400"}`}>{status.msg}</p>
        )}
...
// =============================================================================
// ADD TO LOG MODAL — FIX: serving vs grams mode, dynamic macro scaling,
// intelligent per_100g fallback, immediate tracker refresh
// =============================================================================
function AddToLogModal({ item, onClose, onAdded }) {
  ...
  // Resolve best available base nutrition data
  const rawNutrition = item.nutrition || {};
  const per100g      = rawNutrition.per_100g    && Object.keys(rawNutrition.per_100g).length > 0
                         ? rawNutrition.per_100g    : null;
  const perServing   = rawNutrition.per_serving && Object.keys(rawNutrition.per_serving).length > 0
                         ? rawNutrition.per_serving : null;

  // FIX: Intelligently select available dataset — never leave user without data
  ...
  // Build the nutrition object to submit — always in per_serving format
  // so the backend _parse_num * servings gives correct totals
  const buildSubmitNutrition = () => {
    if (mode === "serving") {
      // Submit the original nutrition; backend multiplies by servings
      return rawNutrition;
    } else {
      // Grams mode: submit pre-scaled values as a single serving
      return {
        per_serving: {
          size: `${gramsNum}g`,
    ...
    try {
      ...
      await apiFetch("/log", {
        ...
        body: JSON.stringify({
          ...
          nutrition: buildSubmitNutrition(),
        }),
      });
      setStatus({ type: "ok", msg: "Added to today's log!" });
      // FIX: Call onAdded immediately to trigger tracker refresh
      onAdded();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/90 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
        ...
        <p className="text-sm text-slate-300 font-bold">{item.name}</p>

        {/* FIX: Mode selector — Per Serving vs Custom Grams */}
        <div className="flex p-1 bg-slate-950 border border-slate-800 rounded-xl">
          {[["serving", "Per Serving"], ["grams", "Custom (g)"]].map(([m, label]) => (
            <button key={m} onClick={() => setMode(m)}
              className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${mode === m ? "bg-emerald-500 text-slate-950 shadow" : "text-slate-500 hover:text-slate-300"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        {mode === "serving" ? (
          <div>
            ...
            {baseNutrition.size && (
              <p className="text-[10px] font-mono text-slate-600 mt-1">Base: {baseNutrition.size}</p>
            )}
          </div>
        ) : (
          <div>
            ...
            <div className="flex items-center gap-3">
              <input type="number" min="1" step="1" value={grams} onChange={e => setGrams(e.target.value)}
                placeholder="e.g. 150"
          ...
          <div className="text-[10px] font-mono leading-relaxed">
            {scalingInfo.warn
              ...
              : <span className="text-slate-600">
                  ...
                  {mode === "grams" && gramsNum > 0 && scalingInfo.factor !== 1 && (
                    <span className="text-emerald-700 ml-2">→ {scalingInfo.targetLabel} | ×{scalingInfo.factor.toFixed(4)}</span>
                  )}
                  {mode === "serving" && servingsNum !== 1 && (
                    <span className="text-emerald-700 ml-2">× {servingsNum} servings</span>
                  )}
                </span>
            }
          </div>
...
// =============================================================================
// TRACKER TAB — FIX: exposed refresh mechanism via refreshKey prop
// =============================================================================
function TrackerTab({ refreshKey }) {
  ...
  // FIX: Re-fetch whenever refreshKey changes (triggered by AddToLogModal onAdded)
  useEffect(() => { loadData(); }, [loadData, refreshKey]);

  const saveGoals = async () => {
    setSavingGoals(true);
    try {
      const updated = await apiFetch("/goals", { method: "POST", body: JSON.stringify(goalDraft) });
      setGoals(updated);
  ...
  const deleteEntry = async (logId) => {
    setDeletingId(logId);
    try {
      await apiFetch(`/log/${logId}`, { method: "DELETE" });
      await loadData();
    } catch (e) {
      console.error(e);
    } finally {
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
            <Target className="h-4 w-4 text-cyan-400" /> Daily Goals
          </h3>
        ...
        {editingGoals ? (
          ...
          <div className="space-y-3">
            ...
            {goals.fibre > 0 && (
              <MacroBar label="Fibre" current={totals.fibre || 0} goal={goals.fibre} color="bg-emerald-400" />
            )}
          </div>
        )}
      </div>
...
// =============================================================================
// LIBRARY TAB (Folders)
// FIX: Enter creates folder, Ctrl+Backspace/Delete word editing,
//      images shown using stored raw_url/processed_url from item metadata
// =============================================================================
function LibraryTab({ onAddToLog }) {
  ...
  // FIX: Enter submits, Ctrl+Backspace deletes last word, Ctrl+Delete deletes next word
  const handleFolderKeyDown = (e) => {
    if (e.key === "Enter") { e.preventDefault(); createFolder(); return; }
    if (e.ctrlKey && e.key === "Backspace") {
      e.preventDefault();
      setNewFolderName(prev => prev.replace(/\s*\S+\s*$/, ""));
      return;
    }
    if (e.ctrlKey && e.key === "Delete") {
      e.preventDefault();
      const input = e.target;
      const pos   = input.selectionStart;
      const after = newFolderName.slice(pos);
      const trimmed = after.replace(/^\s*\S+/, "");
      setNewFolderName(newFolderName.slice(0, pos) + trimmed);
    }
  };
  ...
  const deleteItem = async (folderId, itemId) => {
    ...
    try {
      ...
      setFolderData(prev => ({
        ...prev,
        [folderId]: { ...prev[folderId], items: prev[folderId].items.filter(i => i.item_id !== itemId) },
      }));
    } catch (e) { console.error(e); }
  ...
  return (
    <div className="space-y-4">
      {/* Create Folder — Enter key support */}
      <div className="flex gap-2">
        ...
        <button onClick={createFolder} disabled={creating || !newFolderName.trim()}
          ...
          {creating ? <Loader2 className="animate-spin h-4 w-4" /> : <FolderPlus className="h-4 w-4" />}
          CREATE
        </button>
      </div>
      ...
      {/* Folder List */}
      {folders.length === 0 ? (
        <div className="text-center py-12 text-slate-700 text-sm italic border border-dashed border-slate-800 rounded-2xl">
            ...
            <div key={folder.folder_id} className="border border-slate-800 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-3 p-4 cursor-pointer hover:bg-slate-800/30 transition-colors"
                ...
                {openFolder === folder.folder_id
                  ...
                  : <Folder className="h-5 w-5 text-slate-500 flex-shrink-0" />}
                <span className="flex-1 text-sm font-bold text-slate-300">{folder.name}</span>
                <span className="text-[10px] font-mono text-slate-600">
                  {folderData[folder.folder_id]?.items?.length ?? ""} items
                </span>
                <button onClick={e => { e.stopPropagation(); deleteFolder(folder.folder_id); }}
                  className="text-slate-700 hover:text-rose-400 transition-colors ml-2">
                ...
                <div className="border-t border-slate-800 divide-y divide-slate-800/50">
                  {folderData[folder.folder_id].items.length === 0 ? (
                    ...
                    folderData[folder.folder_id].items.map(item => {
                      ...
                      // FIX: Use backend-provided image URLs for persistent high-quality previews
                      const imageUrl = item.nutrition?.processed_url || item.nutrition?.raw_url
                                    || item.processed_url || item.raw_url || null;

                      return (
                        <div key={item.item_id} className="flex items-center gap-3 p-3 pl-4 hover:bg-slate-800/20 transition-colors">
                          {/* FIX: Show persistent image preview from S3 URL if available */}
                          {imageUrl ? (
                          ...
                          <button onClick={() => deleteItem(folder.folder_id, item.item_id)}
...
function NutrientGrid({ data, activeTab, per100gData }) {
  ...
  if (activeTab === "per_100g") { baseGrams = 100; baseLabel = "100g"; }
    ...
    if (sg) { baseGrams = sg; baseLabel = `${data.size} (${sg}g)`; }
      ...
      baseGrams = 100; scaleFrom = per100gData; baseLabel = "100g (fallback)";
    ...
    <div className="space-y-5">
      <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Custom Serving Calculator</span>
        </div>
...
// =============================================================================
// SCAN TAB
// FIX: images stored with raw_url/processed_url from backend response;
//      previews use backend URL when available (persistent across navigation)
//      kJ→kcal normalization applied to results at ingest time
// =============================================================================
function ScanTab({ onAddToLog }) {
  ...
  const [saveModal, setSaveModal]           = useState(null); // { result, imageId }
  ...
  const handleCropConfirm = useCallback(async (cropData) => {
    ...
    // FIX: store localPreview separately so we can replace with backend URL later
    setImages(prev => [...prev, { file, preview, localPreview: preview, cropData, persistentUrl: null }]);
  ...
  const handleCropCancel = useCallback(() => { setCropperQueue([]); setCropperFile(null); }, []);
  ...
  const removeImage = useCallback((index) => {
    setImages(prev => {
      ...
      // Only revoke if it's a local blob URL, not a persistent backend URL
  ...
  const switchToIndex = useCallback((i, resultArr) => {
    ...
    // FIX: Intelligently select the available tab — prefer per_100g, fall back to per_serving
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (filesToSend.length === 1) {
        ...
        // FIX: Apply kJ→kcal normalization at ingest time
        const arr = (Array.isArray(data) ? data : [data]).map(normalizeResult);
        setResults(arr);
        // FIX: Update image entries with persistent backend URLs
        ...
        const arr = (Array.isArray(data) ? data : [data]).map(normalizeResult);
        setResults(arr);
        // FIX: Update all image entries with backend URLs
        setImages(prev => prev.map((img, idx) => ({
          ...img,
          persistentUrl: arr[idx]?.processed_url || arr[idx]?.raw_url || null,
          preview: arr[idx]?.processed_url || arr[idx]?.raw_url || img.preview,
  ...
  // FIX: prefer persistentUrl (backend S3) over local blob preview
        ...
        <div className="lg:col-span-4 space-y-4">
                  ...
                  return (
                    <div key={i} onClick={() => setActiveIndex(i)}
                      ...
                      {thumbSrc
                        ...
                        : <div className="w-full h-full bg-slate-800 flex items-center justify-center"><Upload className="h-4 w-4 text-slate-600" /></div>
                      }
                      {!results && (
                        <>
          ...
          <button onClick={handleAnalyze} disabled={loading || images.length === 0 || !!cropperFile}
            ...
            {loading ? <Loader2 className="animate-spin h-5 w-5" />
              : results ? <RefreshCcw className="h-5 w-5" />
          ...
          {/* FIX: Quick log button from scan results */}
          {currentResult && (
            <button
              onClick={() => onAddToLog({ name: `Label ${activeIndex + 1}`, nutrition: currentResult })}
                      ...
                      return (
                        <button key={i} onClick={() => switchToIndex(i, results)}
                          ...
                          <div className={`w-10 h-10 rounded-lg overflow-hidden border flex-shrink-0 ${activeIndex === i ? "border-cyan-500/50" : "border-slate-700"}`}>
                            {thumbSrc
                              ? <img src={thumbSrc} alt={`Label ${i + 1}`} className="w-full h-full object-cover" />
              ...
              <div className="flex items-center gap-3 bg-slate-950/50 border border-slate-800 rounded-2xl p-3">
                <div className="w-16 h-16 rounded-xl overflow-hidden border border-slate-700 flex-shrink-0 bg-slate-800">
                  {currentPreview
                    ...
                    : <div className="w-full h-full flex items-center justify-center"><Database className="h-4 w-4 text-slate-700" /></div>
                  }
                </div>
                <div className="flex-1">
                  ...
                  {images[activeIndex]?.cropData && <div className="text-[9px] font-mono text-cyan-800 mt-0.5">✂ CROPPED · GRAYSCALE · OPTIMIZED</div>}
                  {images[activeIndex]?.persistentUrl && <div className="text-[9px] font-mono text-emerald-900 mt-0.5">☁ PERSISTED</div>}
                </div>
                <button
                  onClick={() => setSaveModal({ result: currentResult, imageId: currentResult?.image_id || "" })}
...
// =============================================================================
// ROOT APP
// FIX: logRefreshKey is incremented on onAdded to instantly refresh TrackerTab
//      ScanTab now receives onAddToLog so items can be logged directly from scan
// =============================================================================
export default function App() {
  ...
  const handleAddToLog = useCallback((item) => {
    setAddToLogItem(item);
  }, []);

  const handleLogAdded = useCallback(() => {
    // FIX: Increment refresh key IMMEDIATELY to force TrackerTab refetch
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans">
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        ...
        {/* Tab Content — FIX: use display:none instead of unmounting to preserve ScanTab state */}
        <div style={{ display: activeMainTab === "scan"    ? "block" : "none" }}>
          <ScanTab onAddToLog={handleAddToLog} />
        </div>
        <div style={{ display: activeMainTab === "library" ? "block" : "none" }}>
          <LibraryTab onAddToLog={handleAddToLog} />
        </div>
